...
module Import
  class ImportTransactionsIncomeCsvService
    EXCLUDED_CATEGORIES = []

    class << self
      def service_for_file?(file, model)
        file_name = File.basename(file.original_filename)
        file_name.match?(/Budget (\d{4}) - Income\.csv/) && model == Transaction
      end
    end
    ...
    def preview
      file_name = File.basename(@file.original_filename)
      file_match = file_name.match(/Budget (\d{4}) - Income\.csv/)

      unless file_match
        raise Errors::InvalidFileError, "File does not match expected format. Expected format: Budget YYYY - Income.csv"
      end
      ...
      csv.each_with_index do |row, index|
        (0..11).each do |month_index|
          ...
          category = Category.find_or_initialize_by(
            name: category_name, 
            category_type: CategoryTypes::CATEGORY_TYPES[:income]
          )
